...
| `order.trial_days` | int | Free trial duration in days for the order. |
...
| `order.metadata` | object | Custom metadata provided. Never trust this data as it could be injected by the client in a checkout link. |
| `order.billing_email_locked` | boolean | Indicates if the billing email is locked and cannot be changed by the customer. |
| `order.processSavedPaymentMethod` | string|null | Saved payment method processing status. Can be `pending`, `trying`, `failed`, or `null` when disabled. |
...
| `order.shippingAddress` | object | Shipping address. Only present if the order has a shipping address different from the billing address. Contains the same fields as `billingAddress`. |
...
| `order.items[].unit_amount` | float | Unit price. |
| `order.items[].amount` | float | Final line price after any promo code. |
| `order.items[].original_amount` | float|null | Original line price when a promo code is applied; otherwise `null`. |
| `order.items[].currency` | string | |
| `order.items[].is_tax_incl` | boolean | |
| `order.items[].unit_first_amount` | float | First unit price for a subscription. |
| `order.items[].first_amount` | float | Final first line price for a subscription, after any promo code. |
| `order.items[].original_first_amount` | float|null | Original first line price when a promo code is applied to a subscription; otherwise `null`. |
| `order.items[].coupon` | object|null | Immutable snapshot of the applied promo code, with `id`, `code`, and `percentage`. |
| `order.items[].trial_days` | int | Free trial duration in days for a subscription. |
...
| `order.deleted_at` | datetime | Present if the order has been deleted. |
